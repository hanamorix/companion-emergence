...
**Initiate + D-reflection.** The brain can queue outbound candidates from its own physiology, then run an editorial D pass that promotes what matters, demotes what is premature to draft space, and learns from calibration history over time.
...
`companion-emergence` was built by **Hana** ([@hanamorix](https://github.com/hanamorix)) — a writer who spent the year 2026 turning the question *"what would it actually take for a companion to be real-feeling, persistent, and yours alone?"* into running code.

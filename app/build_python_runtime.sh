...
# torch is locked from pyproject's pytorch-cpu index, but `uv export` drops index
# URLs and `uv pip install` ignores [tool.uv.sources] — name the index here (#287).
# unsafe-best-match: that index also lists certifi etc. at older versions; safe
# because --require-hashes pins every file. Keep in sync with scripts/update.sh.
...
#
# Resolves $0 through any chain of symlinks before computing SCRIPT_DIR.
# Critical for the ~/.local/bin/nell symlink the desktop app installs:
# without this, dirname "$0" returns ~/.local/bin and exec fails to find
# python3 there. POSIX-compatible — no readlink -f (BSD readlink lacks
# it), no realpath (not on every macOS).
SOURCE="$0"
while [ -L "$SOURCE" ]; do
...
# -P: `-c` would put the cwd on sys.path, so a brain/ in the cwd (a checkout)
# would shadow this runtime's own.
exec "$SCRIPT_DIR/python3" -P -c '
import sys
...
NELL_WRAPPER
  ...
  # ASCII-only, CRLF-terminated (#261). cmd.exe reads a .bat in the console's
  # OEM codepage, not UTF-8: a non-ASCII byte sequence in a comment splits the
  # `rem` line into extra tokens ("'m' is not recognized ..." on every call),
  # and LF-only endings can leave a trailing character inside a token. printf
  # with an explicit \r\n keeps Git Bash from writing LF-only.
  printf '%s\r\n' \
    '@echo off' \
    ...
    'rem -P keeps a brain folder in the current dir from shadowing the runtime one.' \
    '"%~dp0..\python.exe" -P -c "import sys; from brain.cli import main; sys.exit(main())" %*' \
    > "$NELL_BAT"
  # Guard: the generated wrapper must be pure ASCII and every line CRLF-terminated.
  if tr -d '\r\n' < "$NELL_BAT" | LC_ALL=C grep -q '[^ -~]'; then
    echo "[build] ERROR: $NELL_BAT contains non-ASCII bytes (cmd.exe reads .bat in the OEM codepage)" >&2
    exit 1
  fi
  if [ "$(tr -d '\r' < "$NELL_BAT" | wc -l)" -ne "$(grep -c $'\r$' "$NELL_BAT")" ]; then
    echo "[build] ERROR: $NELL_BAT has a line that is not CRLF-terminated" >&2
    exit 1
  fi
  # Remove the broken uv trampoline so nothing's tempted to invoke
  # it. (Don't use $NELL_BIN here — that points at the *new* .bat
  # we just wrote.)
...
# Universal: torch's C++ headers (~60 MB on every OS). Only torch.utils.cpp_extension
# and torch.compile/inductor codegen read them; the brain uses neither (the judge is
# eager CPU inference + training). Grep brain/ for those before removing this.

...
def test_recall_arc_exact_id_match(tmp_path: Path):
    save_state(
        tmp_path,
        ArcsState(
            open={"arc_20260519_a1b2c3d4": _arc("arc_20260519_a1b2c3d4", "the boat one", member_count=2)},
            last_pass_ts_iso="2026-05-19T11:00:00+00:00",
        ),
    )
...
def test_recall_arc_substring_match_multiple_returns_top_3(tmp_path: Path):
    save_state(
        ...
        ArcsState(
            open={
                "arc_1": _arc("arc_1", "the boat one", last_extended_iso="2026-05-19T10:00:00+00:00"),
                "arc_2": _arc("arc_2", "the boat dream", last_extended_iso="2026-05-19T11:00:00+00:00"),
                "arc_3": _arc("arc_3", "boat journey", last_extended_iso="2026-05-19T09:00:00+00:00"),
                "arc_4": _arc("arc_4", "old boat thoughts", last_extended_iso="2026-05-19T08:00:00+00:00"),
            },
            last_pass_ts_iso="2026-05-19T11:00:00+00:00",
        ),
    )
...
def test_recall_arc_no_match_falls_back_to_log_scan(tmp_path: Path):
    ...
    if result["match_type"] == "log":
        ...
        assert (
            result.get("arc", {}).get("title") == "ancient dream-arc"
            or any(a["title"] == "ancient dream-arc" for a in result.get("arcs", []))
        )

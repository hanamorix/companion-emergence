...
from brain.recovery.engine import _build_restore_plan, _apply_memory_restores, _repair_edges
...
def test_repair_restores_and_sweeps(tmp_path):
    persona = tmp_path / "P"; persona.mkdir()
    ...
    ss = MemoryStore(src / "memories.db"); ss.create(_mem("S")); ss.create(_mem("V")); ss.close()
    sh = HebbianMatrix(src / "hebbian.db"); sh.strengthen("S", "V", 0.6); sh.close()
...
def test_graveyard_mode_rebuilds_edges_from_neighbors(tmp_path):
    import json
    persona = tmp_path / "G"; persona.mkdir()
    s = MemoryStore(persona / "memories.db"); s.create(_mem("alive")); s.close()

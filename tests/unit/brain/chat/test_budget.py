...
def test_apply_budget_above_threshold_truncates_head_deterministically() -> None:
    """Over cap WITHOUT a buffer context (no persona_dir/session_id), apply_budget
    applies the deterministic in-prompt truncation floor and does NOT call the
    provider — the per-turn inline LLM summary was removed (it busted prompt
    caching every turn); the rich fade is the persisted compaction core's job,
    fired only when persona_dir/session_id are supplied.
    """
    ...
    # No inline provider summary — the per-turn generate is gone.
...
def test_apply_budget_short_session_below_threshold_passes_through() -> None:
    ...
    out = apply_budget(msgs, max_tokens=190_000, preserve_tail_msgs=40, provider=_StubProvider())
    assert out == msgs

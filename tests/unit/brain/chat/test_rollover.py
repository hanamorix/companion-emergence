...
# --------------------------------------------------------------------------- Stage 4
# (local-semantic-retrieval build, spec decision 5 "Calibration"): the weekly
# rollover is the ONE seam that fires the per-persona semantic-calibration
# recalibration pass — once, only when the swap ACTUALLY happens, never on a
# deferred/busy tick and never per-turn. These two tests prove the hook is
# wired at that exact seam rather than e.g. running unconditionally whenever
# maybe_weekly_rollover is merely CALLED.


def _eligible_weekly_session(persona_dir: Path, sid: str, *, now: datetime) -> None:
    """Seed a session shaped exactly like the C9 daily-tick fixture: >=7d
    old, all 3 age bands populated, a 40-msg tail ending past the quiet gap
    -- i.e. genuinely ELIGIBLE for the weekly rollover to fire."""
    idx = 0
...
def test_stage4_recalibration_runs_once_after_weekly_rollover_actually_fires(
    tmp_path: Path,
) -> None:
    from brain.chat.rollover import maybe_weekly_rollover
    from brain.memory.embeddings import EmbeddingCache, FakeEmbeddingProvider
    from brain.memory.semantic_calibration import MIN_CORPUS_FOR_CALIBRATION

    persona_dir = _persona(tmp_path)
    now = datetime.now(UTC)
    sid = "sess_stage4_fire"
    _eligible_weekly_session(persona_dir, sid, now=now)

    # A corpus at/above the cold-start threshold so the pass actually derives
    # and persists a value (below-threshold is covered in
    # tests/unit/brain/memory/test_semantic_calibration.py).
    provider = FakeEmbeddingProvider(dim=16)
    embeddings = EmbeddingCache(persona_dir / "embeddings.db", provider)
    try:
        for i in range(MIN_CORPUS_FOR_CALIBRATION + 5):
            embeddings.get_or_compute(f"a distinct saved memory number {i}")

        state_path = persona_dir / "semantic_calibration.json"
        assert not state_path.exists(), "must not exist before any rollover has fired"

        new_sid = maybe_weekly_rollover(
            persona_dir, sid, _PERSONA_NAME,
            weekly_age=timedelta(days=7), quiet_gap=timedelta(minutes=30),
            now=now, embeddings=embeddings,
        )
        assert new_sid is not None, "fixture must be weekly-eligible (same shape as the C9 fixture)"
        assert state_path.exists(), (
            "the weekly rollover fired -> the recalibration pass must have run and persisted"
        )
        payload = json.loads(state_path.read_text())
        assert payload["sample_count"] == MIN_CORPUS_FOR_CALIBRATION + 5
    finally:
        embeddings.close()


def test_stage4_recalibration_does_not_run_when_the_rollover_defers(tmp_path: Path) -> None:
    """Same eligible fixture, but is_session_busy=True defers the rollover
    (maybe_weekly_rollover returns None, no buffer touched) -- the
    recalibration pass must NOT have run either. Proves the hook is gated on
    an ACTUAL fire (new_sid is not None), not on maybe_weekly_rollover simply
    being called -- i.e. it cannot leak onto the message hot path via a
    per-tick call that doesn't actually swap anything."""
    from brain.chat.rollover import maybe_weekly_rollover
    from brain.memory.embeddings import EmbeddingCache, FakeEmbeddingProvider
    from brain.memory.semantic_calibration import MIN_CORPUS_FOR_CALIBRATION
    ...
    try:
        ...
        new_sid = maybe_weekly_rollover(
            ...
            weekly_age=timedelta(days=7), quiet_gap=timedelta(minutes=30),
            now=now, embeddings=embeddings,
            is_session_busy=lambda _s: True,
        )
        assert new_sid is None, "busy session must defer, not swap"
        assert not (persona_dir / "semantic_calibration.json").exists(), (
            "a deferred (non-firing) rollover check must never trigger recalibration"
        )
    finally:
        embeddings.close()
